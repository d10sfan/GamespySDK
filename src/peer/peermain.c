...
void peerCreateStagingRoomWithSocketA(PEER peer,
                                      const char* name,
                                      int maxPlayers,
                                      ...
                                      void* param,
                                      PEERBool blocking);
void peerStartAutoMatchWithSocketA(PEER peer,
                                   int maxPlayers,
                                   const char* filter,
                                   SOCKET socket,
                                   unsigned short port,
...
** GENERAL **
************/
static PEERBool piCheckBlockingID(PEER peer, int ID)
{
...
void peerConnectLoginA(PEER peer,
                       int namespaceID,
                       const char* email,
                       ...
                       PEERBool blocking)
{
    ...
    if (success) {
        ...
        if (!piNewConnectOperation(peer,
                                   (uniquenick && uniquenick[0]) ? PI_CONNECT_UNIQUENICK_LOGIN
...
#ifdef GSI_UNICODE
void peerConnectLoginW(PEER peer,
                       int namespaceID,
...
void peerConnectPreAuthA(PEER peer,
...
#ifdef GSI_UNICODE
void peerConnectPreAuthW(PEER peer,
...
#ifdef GSI_UNICODE
void peerRetryWithNickW(PEER peer, const unsigned short* nick)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
void peerRegisterUniqueNickA(PEER peer, int namespaceID, const char* uniquenick, const char* cdkey)
{
    PEER_CONNECTION;
...
#ifdef GSI_UNICODE
void peerRegisterUniqueNickW(PEER peer, int namespaceID, const unsigned short* uniquenick, const unsigned short* cdkey)
{
    char* uniquenick_A = UCS2ToUTF8StringAlloc(uniquenick);
...
PEERBool peerSetTitleA(PEER peer,
                       const char* title,
...
#ifdef GSI_UNICODE
PEERBool peerSetTitleW(PEER peer,
                       ...
                       PEERBool crossPingRooms[NumRooms])
{
    ...
    PEERBool result = peerSetTitleA(peer,
                                    title_A,
                                    qrSecretKey_A,
                                    sbName_A,
                                    sbSecretKey_A,
...
#ifdef _DEBUG
static void piNumPlayersConsistencyCheckMap(piPlayer* player, int count[NumRooms])
{
    int i;
...
#else
void peerFixNickW(unsigned short* newNick, const unsigned short* oldNick)
{
    chatFixNickW(newNick, oldNick);
}
const unsigned short* peerTranslateNickW(unsigned short* nick, const unsigned short* extension)
{
...
void peerChangeNickA(PEER peer, const char* newNick, peerChangeNickCallback callback, void* param, PEERBool blocking)
{
    PEERBool success = PEERTrue;
...
#ifdef GSI_UNICODE
void peerChangeNickW(PEER peer,
...
void peerStayInRoom(PEER peer, RoomType roomType)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
void peerSetQuietMode(PEER peer, PEERBool quiet)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerSetAwayModeW(PEER peer, const unsigned short* reason)
{
...
#endif
*/
void peerAuthenticateCDKeyA(PEER peer,
                            const char* cdkey,
...
#ifdef GSI_UNICODE
void peerAuthenticateCDKeyW(PEER peer,
...
void peerSendNatNegotiateCookie(PEER peer, unsigned int ip, unsigned short port, int cookie)
{
    piSendNatNegotiateCookie(peer, ip, port, cookie);
}

void peerSendMessageToServer(PEER peer, unsigned int ip, unsigned short port, const char* data, int len)
{
    piSendMessageToServer(peer, ip, port, data, len);
}

void peerAlwaysGetPlayerInfo(PEER peer, PEERBool always)
{
    PEER_CONNECTION;
...
** ROOMS **
**********/
void peerJoinTitleRoomA(PEER peer,
                        const char password[PEER_PASSWORD_LEN],
                        peerJoinRoomCallback callback,
                        void* param,
                        PEERBool blocking)
...
#ifdef GSI_UNICODE
void peerJoinTitleRoomW(PEER peer,
...
static void piJoinStagingRoom(PEER peer,
                              SBServer server,
                              const char* channel,
                              const char password[PEER_PASSWORD_LEN],
                              peerJoinRoomCallback callback,
...
void peerJoinStagingRoomA(PEER peer,
                          SBServer server,
                          const char password[PEER_PASSWORD_LEN],
                          peerJoinRoomCallback callback,
                          void* param,
...
#ifdef GSI_UNICODE
void peerJoinStagingRoomW(PEER peer,
                          SBServer server,
                          const unsigned short password[PEER_PASSWORD_LEN],
                          peerJoinRoomCallback callback,
                          void* param,
...
void peerJoinStagingRoomByChannelA(PEER peer,
                                   const char* channel,
                                   const char password[PEER_PASSWORD_LEN],
                                   peerJoinRoomCallback callback,
                                   void* param,
...
#ifdef GSI_UNICODE
void peerJoinStagingRoomByChannelW(PEER peer,
...
void peerCreateStagingRoomA(PEER peer,
                            const char* name,
                            int maxPlayers,
                            const char password[PEER_PASSWORD_LEN],
                            peerJoinRoomCallback callback,
...
#ifdef GSI_UNICODE
void peerCreateStagingRoomW(PEER peer,
                                      ...
                                      PEERBool blocking)
{
    ...
    ///////////////////////
    if (success
        && !piNewCreateStagingRoomOperation(peer, name, password, maxPlayers, socket, port, callback, param, opID))
        success = PEERFalse;
...
#ifdef GSI_UNICODE
void peerCreateStagingRoomWithSocketW(PEER peer,
                                      const unsigned short* name,
                                      int maxPlayers,
                                      const unsigned short password[PEER_PASSWORD_LEN],
                                      SOCKET socket,
...
void peerLeaveRoomA(PEER peer, RoomType roomType, const char* reason)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerLeaveRoomW(PEER peer, RoomType roomType, const unsigned short* reason)
{
    if (reason != NULL) {
        char* reason_A = UCS2ToUTF8StringAlloc(reason);
...
void peerListGroupRoomsA(PEER peer,
                         const char* fields,
...
#ifdef GSI_UNICODE
void peerListGroupRoomsW(PEER peer,
...
void peerStartListingGamesA(PEER peer,
...
#ifdef GSI_UNICODE
void peerStartListingGamesW(PEER peer,
                            const unsigned char* fields,
                            int numFields,
...
void peerMessageRoomA(PEER peer, RoomType roomType, const char* message, MessageType messageType)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerMessageRoomW(PEER peer, RoomType roomType, const unsigned short* message, MessageType messageType)
{
...
void peerUTMRoomA(PEER peer, RoomType roomType, const char* command, const char* parameters, PEERBool authenticate)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
void peerSetPasswordA(PEER peer, RoomType roomType, const char password[PEER_PASSWORD_LEN])
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerSetPasswordW(PEER peer, RoomType roomType, const unsigned short password[PEER_PASSWORD_LEN])
{
    char* password_A = UCS2ToUTF8StringAlloc(password);
...
void peerSetRoomNameA(PEER peer, RoomType roomType, const char* name)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerSetRoomNameW(PEER peer, RoomType roomType, const unsigned short* name)
{
    char* name_A = UCS2ToUTF8StringAlloc(name);
...
const char* peerGetRoomNameA(PEER peer, RoomType roomType)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
const unsigned short* peerGetRoomNameW(PEER peer, RoomType roomType)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
const char* peerGetRoomChannelA(PEER peer, RoomType roomType)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
const unsigned short* peerGetRoomChannelW(PEER peer, RoomType roomType)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
PEERBool peerInRoom(PEER peer, RoomType roomType)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
static void piEnumPlayersEnumRoomPlayersCallback(PEER peer, RoomType roomType, piPlayer* player, int index, void* param)
{
...
#ifdef GSI_UNICODE
void peerMessagePlayerW(PEER peer, const unsigned short* nick, const unsigned short* message, MessageType messageType)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
    char* message_A = UCS2ToUTF8StringAlloc(message);
...
void peerUTMPlayerA(PEER peer, const char* nick, const char* command, const char* parameters, PEERBool authenticate)
{
    // Send it.
    ///////////
...
void peerKickPlayerA(PEER peer, RoomType roomType, const char* nick, const char* reason)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerKickPlayerW(PEER peer, RoomType roomType, const unsigned short* nick, const unsigned short* reason)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
    char* reason_A = UCS2ToUTF8StringAlloc(reason);
...
PEERBool peerGetPlayerPingA(PEER peer, const char* nick, int* ping)
{
    piPlayer* player;
...
#ifdef GSI_UNICODE
PEERBool peerGetPlayerPingW(PEER peer, const unsigned short* nick, int* ping)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
PEERBool peerGetPlayersCrossPingA(PEER peer, const char* nick1, const char* nick2, int* crossPing)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
PEERBool peerGetPlayersCrossPingW(PEER peer, const unsigned short* nick1, const unsigned short* nick2, int* crossPing)
{
...
PEERBool peerPingPlayerA(PEER peer, const char* nick)
{
    piPlayer* player;
...
#ifdef GSI_UNICODE
PEERBool peerPingPlayerW(PEER peer, const unsigned short* nick)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
PEERBool peerGetPlayerInfoNoWaitA(PEER peer, const char* nick, unsigned int* IP, int* profileID)
{
    piPlayer* player;
    ...
    if (!player || !player->gotIPAndProfileID) {
        ...
        ///////////////////////////
        if (chatGetBasicUserInfoNoWaitA(connection->chat, nick, &info, NULL)
            && piDemangleUser(info, &locIP, &locProfileID)) {
            if (player)
...
#ifdef GSI_UNICODE
PEERBool peerGetPlayerInfoNoWaitW(PEER peer, const unsigned short* nick, unsigned int* IP, int* profileID)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
void peerGetPlayerInfoA(PEER peer, const char* nick, peerGetPlayerInfoCallback callback, void* param, PEERBool blocking)
{
    PEERBool success = PEERTrue;
    piPlayer* player;
    PI_OP_ID;
...
#ifdef GSI_UNICODE
void peerGetPlayerInfoW(PEER peer,
                        const unsigned short* nick,
...
void peerGetPlayerProfileIDA(PEER peer,
                             const char* nick,
...
#ifdef GSI_UNICODE
void peerGetPlayerProfileIDW(PEER peer,
                             const unsigned short* nick,
                             peerGetPlayerProfileIDCallback callback,
                             void* param,
                             PEERBool blocking)
...
void peerGetPlayerIPA(PEER peer, const char* nick, peerGetPlayerIPCallback callback, void* param, PEERBool blocking)
{
    PEERBool success = PEERTrue;
    piPlayer* player;
    PI_OP_ID;
...
#ifdef GSI_UNICODE
void peerGetPlayerIPW(PEER peer,
                      const unsigned short* nick,
...
PEERBool peerIsPlayerHostA(PEER peer, const char* nick, RoomType roomType)
{
    piPlayer* player;
...
#ifdef GSI_UNICODE
PEERBool peerIsPlayerHostW(PEER peer, const unsigned short* nick, RoomType roomType)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
PEERBool peerGetPlayerFlagsA(PEER peer, const char* nick, RoomType roomType, int* flags)
{
    piPlayer* player;
...
#ifdef GSI_UNICODE
PEERBool peerGetPlayerFlagsW(PEER peer, const unsigned short* nick, RoomType roomType, int* flags)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
PEERBool peerGetReadyA(PEER peer, const char* nick, PEERBool* ready)
{
    piPlayer* player;
...
#ifdef GSI_UNICODE
PEERBool peerGetReadyW(PEER peer, const unsigned short* nick, PEERBool* ready)
{
    char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
static void piAreAllReadyEnumRoomPlayersCallback(PEER peer, RoomType roomType, piPlayer* player, int index, void* param)
{
    if (player) {
...
PEERBool peerAreAllReady(PEER peer)
{
    PEERBool allReady;
...
#ifdef GSI_UNICODE
void peerStartGameW(PEER peer, const unsigned short* message, int reportingOptions)
{
    char* message_A = UCS2ToUTF8StringAlloc(message);
...
PEERBool peerStartReporting(PEER peer)
{
    return peerStartReportingWithSocket(peer, INVALID_SOCKET, 0);
}

PEERBool peerStartReportingWithSocket(PEER peer, SOCKET socket, unsigned short port)
{
    PEER_CONNECTION;
...
PEERBool peerIsPlaying(PEER peer)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
void piSendChannelUTM(PEER peer,
                      const char* channel,
                      const char* command,
...
** KEYS **
*********/
void peerSetGlobalKeysA(PEER peer, int num, const char** keys, const char** values)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerSetGlobalKeysW(PEER peer, int num, const unsigned short** keys, const unsigned short** values)
{
...
void peerGetPlayerGlobalKeysA(PEER peer,
                              const char* nick,
                              int num,
                              const char** keys,
...
#ifdef GSI_UNICODE
void peerGetPlayerGlobalKeysW(PEER peer,
                              const unsigned short* nick,
                              int num,
...
void peerGetRoomGlobalKeysA(PEER peer,
                            RoomType roomType,
                            int num,
                            const char** keys,
                            peerGetGlobalKeysCallback callback,
...
#ifdef GSI_UNICODE
void peerGetRoomGlobalKeysW(PEER peer,
                            RoomType roomType,
                            int num,
                            const unsigned short** keys,
                            peerGetGlobalKeysCallback callback,
...
void peerSetRoomKeysA(PEER peer, RoomType roomType, const char* nick, int num, const char** keys, const char** values)
{
    PEER_CONNECTION;
    PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerSetRoomKeysW(PEER peer,
                      RoomType roomType,
                      const unsigned short* nick,
                      int num,
                      const unsigned short** keys,
...
void peerGetRoomKeysA(PEER peer,
                      RoomType roomType,
                      const char* nick,
                      int num,
                      const char** keys,
...
#ifdef GSI_UNICODE
void peerGetRoomKeysW(PEER peer,
                      RoomType roomType,
                      const unsigned short* nick,
                      int num,
                      const unsigned short** keys,
...
void peerSetGlobalWatchKeysA(PEER peer, RoomType roomType, int num, const char** keys, PEERBool addKeys)
{
    piSetGlobalWatchKeys(peer, roomType, num, keys, addKeys);
}
#ifdef GSI_UNICODE
void peerSetGlobalWatchKeysW(PEER peer, RoomType roomType, int num, const unsigned short** keys, PEERBool addKeys)
{
    char** keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
    int i;
...
void peerSetRoomWatchKeysA(PEER peer, RoomType roomType, int num, const char** keys, PEERBool addKeys)
{
    piSetRoomWatchKeys(peer, roomType, num, keys, addKeys);
}
#ifdef GSI_UNICODE
void peerSetRoomWatchKeysW(PEER peer, RoomType roomType, int num, const unsigned short** keys, PEERBool addKeys)
{
    char** keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
    int i;
    peerSetGlobalWatchKeysA(peer, roomType, num, (const char**)keys_A, addKeys);
...
const char* peerGetGlobalWatchKeyA(PEER peer, const char* nick, const char* key)
{
    return piGetGlobalWatchKeyA(peer, nick, key);
}
#ifdef GSI_UNICODE
const unsigned short* peerGetGlobalWatchKeyW(PEER peer, const unsigned short* nick, const unsigned short* key)
{
    return piGetGlobalWatchKeyW(peer, nick, key);
}
#endif

const char* peerGetRoomWatchKeyA(PEER peer, RoomType roomType, const char* nick, const char* key)
{
    return piGetRoomWatchKeyA(peer, roomType, nick, key);
}
#ifdef GSI_UNICODE
const unsigned short*
peerGetRoomWatchKeyW(PEER peer, RoomType roomType, const unsigned short* nick, const unsigned short* key)
{
    return piGetRoomWatchKeyW(peer, roomType, nick, key);
}
#endif

void peerStartAutoMatchA(PEER peer,
                         int maxPlayers,
                         const char* filter,
                         peerAutoMatchStatusCallback statusCallback,
                         peerAutoMatchRateCallback rateCallback,
                         ...
                         PEERBool blocking)
{
    peerStartAutoMatchWithSocketA(
        peer, maxPlayers, filter, INVALID_SOCKET, 0, statusCallback, rateCallback, param, blocking);
}
#ifdef GSI_UNICODE
void peerStartAutoMatchW(PEER peer,
                         int maxPlayers,
                         const unsigned short* filter,
                         peerAutoMatchStatusCallback statusCallback,
                         peerAutoMatchRateCallback rateCallback,
...
#ifdef GSI_UNICODE
void peerStartAutoMatchWithSocketW(PEER peer,
                                   int maxPlayers,
                                   const unsigned short* filter,
                                   SOCKET socket,
                                   unsigned short port,
                                   ...
                                   PEERBool blocking)
{
    char* filter_A = UCS2ToUTF8StringAlloc(filter);
    peerStartAutoMatchWithSocketA(
        peer, maxPlayers, filter_A, socket, port, statusCallback, rateCallback, param, blocking);
    gsifree(filter_A);
}
#endif
